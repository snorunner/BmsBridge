...
public sealed class DanfossReadVarOutOperation : DanfossBaseDeviceOperation
{
    public override string Name => "read_var_out";
    private readonly IEnumerable<(string node, string mod, string point)> _addresses;

    public DanfossReadVarOutOperation(Uri endpoint, IEnumerable<(string node, string mod, string point)> addresses, ILoggerFactory loggerFactory)
